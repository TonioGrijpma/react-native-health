...
{
    HKQuantityType *quantityType = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierWalkingStepLength];
    ...
    [self fetchQuantitySamplesOfType:quantityType
                          ...
                          completion:^(NSArray *results, NSError *error) {
                              if(results){
                                  ...
                                  NSLog(@"error getting step lenght samples: %@", error);
                                  callback(@[RCTMakeError(@"error getting step lenght samples:", error, nil)]);
                                  return;
                              }
                          }];
}
